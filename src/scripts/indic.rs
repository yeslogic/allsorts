...
impl BasicFeature {
    ...
    fn mask(self) -> FeatureMask {
        match self {
            ...
            // All other basic features are enabled for the entire glyph buffer
            _ => FeatureMask::empty(),
        }
    }
}
...
bitflags! {
    struct FeatureMask: u8 {
        // Basic
        const RPHF = 0b0000_0001;
        const PREF = 0b0000_0010;
        const BLWF = 0b0000_0100;
        const HALF = 0b0000_1000;
        const PSTF = 0b0001_0000;
        // Presentation
...
fn to_indic_syllables(
    ...
    let cs: Vec<char> = glyphs
        .iter()
        .map(|g| match g.glyph_origin {
            GlyphOrigin::Char(c) => Ok(c),
            GlyphOrigin::Direct => Err(IndicError::UnexpectedGlyphOrigin),
        })
        .collect::<Result<_, _>>()?;

    let mut cs_slice = &cs[..];
    let mut gs_slice = &glyphs[..];
    while !cs_slice.is_empty() {
        let len = match match_syllable(cs_slice) {
        ...
        // Advance the slices in lockstep
        cs_slice = &cs_slice[len..];
