...
impl<'a> ReadFrom<'a> for ChainHeader {
    ...
    fn from((default_flags, chain_length, n_feature_entries, n_subtables): (u32, u32, u32, u32)) -> Self {
        ChainHeader {
            default_flags,
            chain_length,
            n_feature_entries,
...
impl<'a> ReadFrom<'a> for Feature {
    ...
    fn from((feature_type, feature_setting, enable_flags, disable_flags): (u16, u16, u32, u32)) -> Self {
        Feature {
            feature_type,
            feature_setting,
            enable_flags,
...
impl<'a> ReadBinary<'a> for Chain {
    ...
    fn read(ctxt: &mut ReadCtxt<'a>) -> Result<Self, ParseError> {
        ...
        let features = ctxt.read_array::<Feature>(usize::try_from(chain_header.n_feature_entries)?)?;
    ...
    fn read(ctxt: &mut ReadCtxt<'a>) -> Result<Self, ParseError> {
        ...
        match subtable_header.coverage & 0xFF {
            ...
            _ => {
                ...
                subtable_body = SubtableType::Other{other_subtable: subtable_vec};
            },
        }
...
impl<'a> ReadFrom<'a> for STXheader {
    ...
    fn from((n_classes, class_table_offset, state_array_offset, entry_table_offset): (u32, u32, u32, u32)) -> Self {
        STXheader {
            n_classes,
            class_table_offset,
            state_array_offset,
...
impl<'a> ReadBinary<'a> for LigatureSubtable {
    ...
    fn read(ctxt: &mut ReadCtxt<'a>) -> Result<Self, ParseError> {
        ...
        let class_table = subtable.offset(usize::try_from(stx_header.class_table_offset)?)
                                    .read::<ClassLookupTable>()?;
                                    
        //read the state array:
        let state_array = subtable.offset(usize::try_from(stx_header.state_array_offset)?)
                                  .read_dep::<StateArray>(NClasses(stx_header.n_classes))?;
        
        
        //read the ligature entry table
        let entry_table = subtable.offset(usize::try_from(stx_header.entry_table_offset)?)
                                  .read::<LigatureEntryTable>()?;
        
        //read the ligature action table
        let action_table = subtable.offset(usize::try_from(lig_action_offset)?)
                                    .read::<LigatureActionTable>()?;
        
        //read the component table
        let component_table = subtable.offset(usize::try_from(component_offset)?)
                                    .read::<ComponentTable>()?;
                                    
        //read the ligature list 
        let ligature_list = subtable.offset(usize::try_from(ligature_list_offset)?)
                                    .read::<LigatureList>()?;
            ...
            0 => {
                match lookup_header.bin_srch_header {
                    None => {
                        ...
                        let lookup_table = LookupTable::Format0 {
                                       lookup_values,
                                   };
            ...
            2 => {
                match lookup_header.bin_srch_header {
                    Some(ref b_sch_header) => {
                        ...
                        let lookup_table = LookupTable::Format2 {
                                                lookup_segments,
                                           };
                ...
                match lookup_header.bin_srch_header {
                    Some(ref b_sch_header) => {
                        ...
                        for i in 0..b_sch_header.n_units {
                            ...
                            let mut read_ctxt = class_table.offset(usize::from(offset_to_lookup_values)).ctxt();
                        ...
                        let lookup_table =     LookupTable::Format4 {
                                                lookup_segments,
                                            };
            ...
            6 => {
                match lookup_header.bin_srch_header {
                    Some(ref b_sch_header) => {
                        ...
                        let lookup_table = LookupTable::Format6 {
                                                lookup_entries,
                                           };
...
pub fn glyph_class(glyph: u16, class_table: &ClassLookupTable) -> u16 {
    ...
    match class_table.lookup_header.format {
        ...
        4 => {
            match &class_table.lookup_table {
                LookupTable::Format4{lookup_segments} => {
                    for lookup_segment in lookup_segments {
                        if (glyph >= lookup_segment.first_glyph) && (glyph <= lookup_segment.last_glyph) {
                            return lookup_segment.lookup_values[usize::from(glyph - lookup_segment.first_glyph)];
                        }
                    }
                    return 1    //out of bounds
                }
        ...
        8 => {
            match &class_table.lookup_table {
                LookupTable::Format8{first_glyph, glyph_count, lookup_values} => {
        ...
        10 => {
            match &class_table.lookup_table {
                LookupTable::Format10{unit_size, first_glyph, glyph_count, lookup_values} => {
                    match lookup_values {
                        UnitSize::OneByte{lookup_values: one_byte_values} => {
                            if (glyph >= *first_glyph) && (glyph <= (*first_glyph + *glyph_count - 1)) {
                        ...
                        UnitSize::TwoByte{lookup_values: two_byte_values} => {
                            if (glyph >= *first_glyph) && (glyph <= (*first_glyph + *glyph_count - 1)) {
...
impl <'a>LigatureSubstitution<'a> {
    ...
    pub fn process_glyphs(&mut self, ligature_subtable: &LigatureSubtable) -> Result<(), ParseError> {
        ...
        'glyphs: loop {
            ...
            'glyph: loop {
                ...
                let entry = &ligature_subtable.entry_table.lig_entries[usize::from(index_to_entry_table)];
                ...
                if entry_flags & PERFORM_ACTION != 0 {            //Perform Action: use the ligActionIndex to process a ligature group
                    ...
                    'stack: loop {
                        ...
                        let index_to_component_table: usize =  match usize::try_from(index_to_component_table) {
                                                                    Ok(index) => index,
                                                                    Err(_err) => return Err(ParseError::BadValue),    
                                                                };
                    
                        index_to_ligature += &ligature_subtable.component_table.component_array[index_to_component_table];
                        ...
                        if action & LAST != 0 {                //this is the last action, so exit the loop 'stack
                            break 'stack;
                        }
                    }
                    //end of loop 'stack
